...
# ------------------------------------------------------------
@timer
def fetch_binance_candles(symbol, interval="1m", limit=1000, startTime=None, retries=3):
    """Fetch OHLCV data from Binance (with retry, cache, and safety).
    
    Args:
        symbol: Trading pair (e.g., 'BTCUSDT')
        interval: Candle interval (e.g., '1m', '5m', '1h', '4h')
        limit: Number of candles to fetch (max 1000)
        startTime: Optional start time in milliseconds (for historical data)
        retries: Number of retries on failure
    """
    key = f"{symbol.lower()}_{interval}_{startTime}" if startTime else f"{symbol.lower()}_{interval}"
    now = time.time()
    df = pd.DataFrame()

    # ✅ Use cache if recent (skip cache if startTime specified)
    if startTime is None and key in _cached_candles:
    ...
    for attempt in range(retries):
        try:
            ...
            # Cache only if no startTime specified
