...
daily_trading_state = {
    ...
    "daily_loss_limit": 10000.0 * 0.02,  # 2% daily max loss
...
async def ai_trade(symbol, df):
    ...
    if decision == "BUY" and confidence < 80:
        parsed["reason"] = f"{parsed.get('reason', '')} | low_conf_{confidence}%"
        parsed["decision"] = "HOLD"
        print(f"⚠️ Suppressing low-confidence BUY for {symbol_u}: {confidence}% < 80% threshold")
...
def start_trade_for_symbol(sym):
    ...
    # 🔥 NEW: Calculate multi-timeframe data for alignment check
    print(f"📊 Gathering multi-timeframe data for {sym}...")
    multi_tf = {}
    try:
        from data_fetcher import get_recent_candles
        tfs = ["1m", "5m", "15m", "1h", "4h"]
        for tf in tfs:
            try:
                df_tf = get_recent_candles(sym, interval=tf, limit=1000)
                if df_tf is not None and not getattr(df_tf, "empty", True):
                    multi_tf[tf] = evaluate_trend(df_tf, indicators=calculate_indicators(df_tf))
            except Exception as e:
                multi_tf[tf] = {"trend": "NEUTRAL", "score": 0.0, "reason": f"tf_error:{e}"}
    except Exception as e:
        print(f"⚠️ Multi-timeframe gather failed for {sym}: {e}")
        multi_tf = {}
    ...
    try:
        ...
        # 🔥 NEW: Volatility filter - skip dead markets only
        if not df.empty:
            recent_high = df['high'].iloc[-20:].max()
            recent_low = df['low'].iloc[-20:].min()
        ...
        MIN_VOLATILITY = 0.005   # 0.5% min volatility (skip dead markets)
        # REMOVED: MAX_VOLATILITY = 0.20 (high vol is often best trading)
        
        if volatility_pct < MIN_VOLATILITY:
            print(f"⚠️ SKIP {sym}: Market too dead (vol={volatility_pct:.2%})")
            return {"ok": False, "error": f"Volatility too low: {volatility_pct:.2%}"}
        
        print(f"✅ Volatility OK: {volatility_pct:.2%} (min: {MIN_VOLATILITY:.2%})")
        ...
        MIN_RISK_PCT = 0.008    # Minimum 0.8% risk (60% wider than before)
        MIN_REWARD_PCT = 0.025  # Minimum 2.5% reward (150% wider - overcome 0.2% fees)
        MIN_RR_RATIO = 2.5      # Enforce minimum 2.5:1 risk-reward ratio
        ...
        if atr and atr > 0:
            atr_pct = atr / entry
            risk_pct = max(atr_pct * 2.0, MIN_RISK_PCT)      # 33% wider SL (was 1.5)
            reward_pct = max(atr_pct * 5.0, MIN_REWARD_PCT)   # 67% wider TP (was 3.0) → 2.5:1 ratio
        ...
        reward_dist = abs(target - entry) / entry
        risk_dist = abs(stop_loss - entry) / entry
        rr_ratio = reward_dist / risk_dist if risk_dist > 0 else 0
        
        if rr_ratio < MIN_RR_RATIO:
            print(f"⚠️ SKIP {sym}: R:R ratio {rr_ratio:.2f} < minimum {MIN_RR_RATIO}")
            return {"ok": False, "error": f"Insufficient risk-reward: {rr_ratio:.2f}"}
        
        print(f"✅ R:R ratio validated: {rr_ratio:.2f} >= {MIN_RR_RATIO}")
        
        # 🔥 NEW: Multi-timeframe alignment check (simplified for SPOT)
        # Only require 1h confirmation, not both 1h and 4h
        tf_alignment_score = 0
        tf_1h_trend = multi_tf.get("1h", {}).get("trend", "NEUTRAL")
        
        if decision == "BUY" and tf_1h_trend == "UP":
            tf_alignment_score = 1
            print(f"   ✅ 1h aligned: {tf_1h_trend}")
        elif decision == "SELL" and tf_1h_trend == "DOWN":
            tf_alignment_score = 1
            print(f"   ✅ 1h aligned: {tf_1h_trend}")
        else:
            print(f"   ⚠️ 1h not aligned: expected {'UP' if decision == 'BUY' else 'DOWN'}, got {tf_1h_trend}")
        
        MIN_TF_ALIGNED = 1
        if tf_alignment_score < MIN_TF_ALIGNED:
            print(f"⚠️ SKIP {sym}: 1h trend not aligned")
            return {"ok": False, "error": f"1h trend not aligned"}
        
        print(f"✅ Multi-TF alignment: 1h confirmed")
...
def close_trade(trade, result):
    ...
    try:
        ...
        # Calculate P&L
        gross_pnl = (exit_price - entry) * size  # Profit/Loss in USD
        entry_fee = entry * size * TRADE_FEE_RATE
        exit_fee = exit_price * size * TRADE_FEE_RATE
        net_pnl = gross_pnl - entry_fee - exit_fee  # Net after both fees

        result["pnl"] = net_pnl
        result["outcome"] = "win" if gross_pnl > 0 else "loss"

        # ✅ CRITICAL: Return the full exit proceeds to wallet
        # At open: deducted (entry * size) + entry_fee
        # At close: return (exit_price * size) - exit_fee
        # This way: original $100 comes back + profit/loss - exit fee
        exit_proceeds = (exit_price * size) - exit_fee
        portfolio["balance"] += exit_proceeds
        
        print(f"💸 Trade Close - Balance update:")
        print(f"   Entry value: ${entry * size:.2f}")
        print(f"   Exit value: ${exit_price * size:.2f}")
        print(f"   Entry fee: ${entry_fee:.4f} (deducted at open)")
        print(f"   Exit fee: ${exit_fee:.4f} (deducted now)")
        print(f"   Gross P&L: ${gross_pnl:.2f}")
        print(f"   Net P&L: ${net_pnl:.2f}")
        print(f"   Exit proceeds returned: ${exit_proceeds:.2f}")
        ...
        print(f"[✅ TRADE CLOSED] {trade['symbol']} | PnL=${result.get('pnl'):.2f} | Outcome={result.get('outcome')}")
        return {"ok": True}
...
def run_paper_trading(symbol):
    ...
    # 🔥 INITIALIZE MULTI-TIMEFRAME DATA (will be refreshed periodically)
    multi_tf = {}
    last_multi_tf_refresh = 0
    multi_tf_refresh_interval = 60  # Refresh every 60 seconds
    
# idhar change kia ha 
    while True:
            try:
                # ===== 📅 CHECK DAILY DRAWDOWN LIMIT =====
                global daily_trading_state
                daily_loss = daily_trading_state["starting_balance"] - portfolio["balance"]
                
                if daily_loss >= daily_trading_state["daily_loss_limit"]:
                    print(f"🛑 Daily loss limit exceeded: ${daily_loss:.2f} >= ${daily_trading_state['daily_loss_limit']:.2f}")
                    print(f"⏸️ Pausing trades for 1 hour...")
                    time.sleep(3600)  # Pause for 1 hour
                    daily_trading_state["starting_balance"] = portfolio["balance"]  # Reset limit
                    continue
                
                # 🔥 REFRESH MULTI-TIMEFRAME DATA (every 60 seconds)
                current_time = time.time()
                if current_time - last_multi_tf_refresh > multi_tf_refresh_interval:
                    try:
                        multi_tf = {}
                        tfs = ["1m", "5m", "15m", "1h", "4h"]  # All timeframes
                        for tf in tfs:
                            try:
                                df_tf = get_recent_candles(symbol_u, interval=tf, limit=1000)
                                if df_tf is not None and not getattr(df_tf, "empty", True):
                                    multi_tf[tf] = evaluate_trend(df_tf, indicators=calculate_indicators(df_tf))
                                else:
                                    multi_tf[tf] = {"trend": "NEUTRAL", "score": 0.0, "reason": "no_data"}
                            except Exception as e:
                                multi_tf[tf] = {"trend": "NEUTRAL", "score": 0.0, "reason": f"error:{e}"}
                        
                        # 🔥 Build summary string without problematic escaping
                        tf_summary = ", ".join([f"{tf}:{multi_tf[tf].get('trend', '?')}" for tf in tfs])
                        print(f"📊 Multi-TF refreshed: {tf_summary}")
                        last_multi_tf_refresh = current_time
                    except Exception as e:
                        print(f"⚠️ Multi-TF refresh failed: {e}")
                ...
                try:
                    open_pos = trade_history.get_open_trade(symbol_u)  # 🔥 ALWAYS refetch fresh from DB
                    if open_pos:
                ...
                if open_pos.get("side", "").upper() == "BUY":
                    ...
                    price = float(price)  # 🔥 Ensure price is float for comparison
                    
                    # 🔥 DEBUG: Print values before comparison
                    print(f"🔍 [DEBUG] Price type: {type(price)}, value: {price}")
                    print(f"🔍 [DEBUG] SL type: {type(stop_loss)}, value: {stop_loss}")
                    print(f"🔍 [DEBUG] Entry type: {type(entry)}, value: {entry}")
                    ...
                    # 🔥 NEW: PARTIAL PROFIT TAKING (PPT) AT 1.5x RISK
                    ppt_level = entry + (target - entry) * 0.6  # 60% of TP distance = 1.5x R
                    ppt_triggered = open_pos.get("ppt_triggered", False)
                    
                    if not ppt_triggered and price >= ppt_level:
                        print(f"🎯🔥 PARTIAL PROFIT TRIGGER! {symbol_u}:")
                        print(f"   Price: {price:.6f} >= PPT level: {ppt_level:.6f}")
                        print(f"   Closing 50% to lock profit...")
                        
                        size = float(open_pos.get("size", 0))
                        partial_size = size * 0.5
                        
                        # Calculate proceeds from 50% close
                        exit_proceeds = (price * partial_size) - (price * partial_size * 0.001)  # 0.1% exit fee
                        portfolio["balance"] += exit_proceeds
                        
                        # Move SL to breakeven for remaining 50%
                        new_sl = entry * 1.0002  # Breakeven + 0.02% spread buffer
                        
                        # Update trade: mark PPT triggered and move SL
                        try:
                            from sqlalchemy import update
                            with engine.begin() as conn:
                                conn.execute(
                            ...
                            # Update global state
                            open_pos["ppt_triggered"] = True
                            open_pos["size"] = partial_size
                            open_pos["stop_loss"] = new_sl
                            
                            print(f"✅ PPT successful: Closed 50% @ {price:.6f}, locked ${exit_proceeds:.2f}")
                            print(f"🛡️ SL moved to breakeven: {new_sl:.6f}")
                            print(f"📈 Trailing 50% to TP with breakeven protection")
                        except Exception as e:
                            print(f"⚠️ PPT update failed: {e}")
                    
                    # ===== 🎯 CHECK TAKE-PROFIT (full TP for remainder) =====
                    ...
                    # ===== 🛑 CHECK STOP-LOSS =====
                    print(f"🔍 [SL CHECK] Price={price:.6f} vs SL={stop_loss:.6f}, Check: {price} <= {stop_loss} = {price <= stop_loss}")
                    ...
                    # ===== 📉 CHECK AI SELL SIGNAL (WITH TREND PROTECTION) =====
                    try:
                        ...
                        if decision == "SELL" and confidence >= 75:
                            # 🔥 NEW: Check if 1h trend is still UP before selling
                            trend_1h = multi_tf.get("1h", {}).get("trend", "NEUTRAL")
                            
                            if trend_1h == "UP":
                                print(f"⚠️ AI SELL signal IGNORED: 1h still in uptrend!")
                                print(f"   AI confidence: {confidence}%")
                                print(f"   Trend: {trend_1h}")
                                print(f"   Letting trade run with PPT/SL protection...")
                                # Don't close - let TP/SL/PPT handle it
                            else:
                                print(f"📉 AI SELL SIGNAL VALID (trend broken)! {symbol_u}:")
                    ...
                    time.sleep(0.5)  # 🔥 REDUCED from 5 seconds to 0.5 seconds for faster SL detection
                
                # ===== 4️⃣ MONITOR OPEN SELL POSITION =====
                elif open_pos.get("side", "").upper() == "SELL":
                    entry = float(open_pos.get("entry", 0))
                    target = float(open_pos.get("target", 0))
                    stop_loss = float(open_pos.get("stop_loss", 0))
                    price = float(price)  # 🔥 Ensure price is float for comparison
                    
                    # Calculate current P&L (for SELL: negative means profit)
                    pnl = (entry - price) / entry * 100  # 🔥 REVERSED for SELL position
                    
                    # 🔥 DEBUG: Print values before comparison
                    print(f"🔍 [SELL] Price type: {type(price)}, value: {price}")
                    print(f"🔍 [SELL] SL type: {type(stop_loss)}, value: {stop_loss}")
                    
                    # 🔍 MONITORING INFO (every 5 seconds)
                    if time.time() % 5 < 1:
                        print(f"📊 {symbol_u} SELL Monitor:")
                        print(f"   Price: {price:.6f}")
                        print(f"   Entry: {entry:.6f}")
                        print(f"   TP: {target:.6f} ({((entry-target)/entry*100):.2f}%)")
                        print(f"   SL: {stop_loss:.6f} ({((entry-stop_loss)/entry*100):.2f}%)")
                        print(f"   Current P&L: {pnl:.2f}%")
                    
                    # ===== 🎯 CHECK TAKE-PROFIT (SELL: price goes DOWN to TP) =====
                    if target > 0 and price <= target:
                        print(f"🎯 TARGET HIT! {symbol_u} (SELL):")
                        print(f"   Price: {price:.6f} <= TP: {target:.6f}")
                        print(f"   P&L: {pnl:.2f}%")
                        try:
                            trade_history.close_trade_by_price(
                    ...
                    # ===== 🛑 CHECK STOP-LOSS (SELL: price goes UP to SL) =====
                    print(f"🔍 [SELL SL CHECK] Price={price:.6f} vs SL={stop_loss:.6f}, Check: {price} >= {stop_loss} = {price >= stop_loss}")
                    if stop_loss > 0 and price >= stop_loss:
                        print(f"🛑 STOP LOSS HIT! {symbol_u} (SELL):")
                        print(f"   Price: {price:.6f} >= SL: {stop_loss:.6f}")
                        print(f"   P&L: {pnl:.2f}%")
                        try:
                            trade_history.close_trade_by_price(
                    ...
                    print(f"⚠️ Unknown side: {open_pos.get('side')}")
                    time.sleep(2)
                    continue
