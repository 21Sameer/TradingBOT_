...
def open_trade(symbol, side, entry, target, stop_loss, size):
    ...
    try:
        ...
        # NOTE: Fee is NOT deducted here - it's already deducted by engine in start_trade_for_symbol()
        trade_value = entry * size
        entry_fee = trade_value * BUY_FEE_RATE  # fee charged on trade open
        
        print(f"ℹ️ Entry fee {entry_fee:.4f} recorded for {symbol} ({side}) (already deducted by engine)")
